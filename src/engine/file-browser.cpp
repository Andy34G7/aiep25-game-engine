...
void FileBrowser::InitializeImGui(void* window, void* renderer) {
    ...
    ImGui_ImplSDL3_InitForSDLRenderer(static_cast<SDL_Window*>(window), static_cast<SDL_Renderer*>(renderer));
...
void FileBrowser::RenderImGuiFrame(FileBrowserState& state) {
    ...
    if (state.main_window_resized) {
        ImGui::SetWindowSize(ImVec2(state.width, state.height), ImGuiCond_Always);
        state.main_window_resized = false;
    } else {
        ImGui::SetWindowSize(ImVec2(state.width, state.height), ImGuiCond_FirstUseEver);
    ...
    if (filebrowser_isopen) {
        ...
        if (std::fabs(currentSize.x - state.width) > threshold ||
            ...
            // Update ratio
            state.ratio_width = currentSize.x / static_cast<float>(state.main_window_width);
            state.ratio_height = currentSize.y / static_cast<float>(state.main_window_height);
        }
...
void FileBrowser::Render(FileBrowserState& state) {
    if (!m_imguiInitialized) {
        std::cerr << "ImGui not initialized! Call InitializeImGui first." << std::endl;
        return;
    }
...
void FileBrowser::UpdateOnWindowResize(FileBrowserState& state, int newWidth, int newHeight) {
    ...
    if (state.width < 200.0f) state.width = 200.0f;
    if (state.height < 150.0f) state.height = 150.0f;
